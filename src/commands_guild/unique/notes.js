...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'ajouter') {
      ...
      if (count.c >= 25) return await interaction.reply({ content: '❌ Maximum 25 notes. Supprimez-en d\'abord.', ephemeral: true });
    ...
    if (sub === 'liste') {
      ...
      if (!notes.length) return await interaction.reply({ content: '📝 Aucune note. Créez-en une avec `/notes ajouter` !', ephemeral: true });
    ...
    if (sub === 'lire') {
      ...
      if (!note) return await interaction.reply({ content: `❌ Note #${id} introuvable.`, ephemeral: true });
    ...
    if (sub === 'modifier') {
      ...
      if (!r.changes) return await interaction.reply({ content: `❌ Note #${id} introuvable.`, ephemeral: true });
      return await interaction.reply({ content: `✅ Note **#${id}** modifiée !`, ephemeral: true });
    }
    ...
    if (sub === 'supprimer') {
      ...
      if (!r.changes) return await interaction.reply({ content: `❌ Note #${id} introuvable.`, ephemeral: true });
      return await interaction.reply({ content: `✅ Note **#${id}** supprimée.`, ephemeral: true });
    }
    ...
    if (sub === 'epingler') {
      ...
      return await interaction.reply({ content: `✅ Note **#${id}** ${note.pinned ? 'désépinglée' : '📌 épinglée'} !`, ephemeral: true });
    }

    if (sub === 'effacer') {
      const count = db.db.prepare('DELETE FROM notes WHERE guild_id=? AND user_id=?').run(guildId, userId);
      return await interaction.reply({ content: `✅ **${count.changes}** note(s) supprimée(s).`, ephemeral: true });
    }
  }
};
