...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'rejoindre') {
      ...
      if (existing) return interaction.reply({ content: '❌ Vous êtes déjà un agent enregistré.', ephemeral: true });
      ...
      if (nameExists) return interaction.reply({ content: '❌ Ce nom de code est déjà pris.', ephemeral: true });
    ...
    if (sub === 'profil') {
      ...
      if (!agent) return interaction.reply({ content: `❌ **${target.username}** n'est pas un agent enregistré.`, ephemeral: true });
      ...
      return interaction.reply({ embeds: [new EmbedBuilder().setColor('#2C3E50')
    ...
    if (sub === 'missions') {
      ...
      if (!agent) return interaction.reply({ content: '❌ Vous n\'êtes pas enregistré. Utilisez `/espion rejoindre`.', ephemeral: true });
    ...
    if (sub === 'mission') {
      const agent = getAgent(userId);
      if (!agent) return interaction.reply({ content: '❌ Vous n\'êtes pas enregistré.', ephemeral: true });
      ...
      if (elapsed < requiredCd) {
        ...
        return interaction.reply({ content: `⏳ Vous êtes en repos. Prochaine mission disponible dans **${Math.floor(remaining / 60)}min ${remaining % 60}s**.`, ephemeral: true });
      }
    ...
    if (sub === 'voler') {
      ...
      if (!targetAgent) return interaction.reply({ content: `❌ <@${target.id}> n'est pas un agent.`, ephemeral: true });

      const cooldown = 3600;
      if (now - agent.last_mission < cooldown) {
        return interaction.reply({ content: `⏳ Vous ne pouvez pas encore effectuer d'opération. Cooldown : **${Math.floor((cooldown - (now - agent.last_mission)) / 60)}min**.`, ephemeral: true });
      }

      if (targetAgent.intel < 10) return interaction.reply({ content: `❌ <@${target.id}> n'a pas assez d'intel à voler.`, ephemeral: true });
      ...
      if (success) {
        ...
        return interaction.reply({ content: `✅ Vol d'intel réussi ! Vous avez dérobé **${stolen} pts d'intel** à <@${target.id}> !` });
      } else {
        ...
        return interaction.reply({ content: `❌ Vol raté ! <@${target.id}> vous a repéré. Amende : **-${fine} ${coin}**.` });
      }
    }
    ...
    if (sub === 'proteger') {
      ...
      if ((u.balance || 0) < cost) return interaction.reply({ content: `❌ La protection coûte **${cost} ${coin}**.`, ephemeral: true });
    ...
    if (sub === 'classement') {
      ...
      if (!top.length) return interaction.reply({ content: '❌ Aucun agent enregistré.', ephemeral: true });
      const medals = ['🥇', '🥈', '🥉'];
      ...
      return interaction.reply({ embeds: [new EmbedBuilder().setColor('#2C3E50').setTitle('🕵️ Classement des Agents').setDescription(desc)] });
    }
  }
};
