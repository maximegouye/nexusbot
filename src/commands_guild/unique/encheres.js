...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'creer') {
      ...
      if ((u.balance || 0) < 50) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: `❌ Il faut **50 ${coin}** pour mettre un article en vente.`, ephemeral: true });
    ...
    if (sub === 'encherir') {
      ...
      if (now > enc.end_time) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Cette enchère est expirée.', ephemeral: true });
      if (enc.seller_id === userId) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Vous ne pouvez pas enchérir sur votre propre article.', ephemeral: true });
      if (enc.highest_bidder === userId) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Vous êtes déjà le plus offrant.', ephemeral: true });

      const minBid = enc.current_price + enc.min_increment;
      if (montant < minBid) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: `❌ La mise minimale est **${minBid} ${coin}** (actuel: ${enc.current_price} + incrément: ${enc.min_increment}).`, ephemeral: true });

      const u = db.getUser(userId, guildId);
      if ((u.balance || 0) < montant) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: `❌ Vous n\'avez que **${u.balance || 0} ${coin}**.`, ephemeral: true });
    ...
    if (sub === 'liste') {
      ...
      if (!encs.length) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '📋 Aucune enchère active. Créez-en une avec `/enchere creer` !', ephemeral: true });
    ...
    if (sub === 'annuler') {
      ...
      if (!enc) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: `❌ Enchère #${id} introuvable ou vous n\'êtes pas le vendeur.`, ephemeral: true });
      if (enc.highest_bidder) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Impossible d\'annuler : quelqu\'un a déjà enchéri. Attendez la fin.', ephemeral: true });
      db.db.prepare('UPDATE encheres SET status=? WHERE id=?').run('annulee', id);
      db.addCoins(userId, guildId, 25); // Remboursement partiel
      return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: `✅ Enchère #${id} annulée. Remboursement partiel : +25 ${coin}`, ephemeral: true });
    }
    ...
    if (sub === 'cloturer') {
      ...
      const enc = db.db.prepare('SELECT * FROM encheres WHERE id=? AND guild_id=?').get(id, guildId);
      if (!enc) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: `❌ Enchère #${id} introuvable.`, ephemeral: true });
      if (enc.status !== 'active') return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Cette enchère est déjà clôturée.', ephemeral: true });
      if (enc.seller_id !== userId && !interaction.member.permissions.has(0x8n)) {
        return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Seul le vendeur ou un admin peut clôturer.', ephemeral: true });
      }
      if (now < enc.end_time) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: `❌ L\'enchère n\'est pas encore terminée. Fin : <t:${enc.end_time}:R>`, ephemeral: true });
      ...
      if (enc.highest_bidder) {
        ...
        return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: `❌ Enchère #${id} terminée sans enchérisseur. Frais remboursés (+50 ${coin}).`, ephemeral: true });
      }
    }
  }
};
