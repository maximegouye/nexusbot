  ...
  async execute(interaction) {
    ...
    if (sub === 'creer') {
      if (!interaction.member.permissions.has(0x8n) && !interaction.member.permissions.has(0x20n)) {
        return interaction.reply({ content: '❌ Seuls les admins et modérateurs peuvent créer des élections.', ephemeral: true });
      }
      ...
      if (candidats.length < 2) return interaction.reply({ content: '❌ Il faut au moins 2 candidats (séparés par des virgules).', ephemeral: true });
    ...
    if (sub === 'voter') {
      ...
      if (elec.status !== 'active' || now > elec.end_time) return interaction.reply({ content: '❌ Cette élection est terminée.', ephemeral: true });

      const candidats = JSON.parse(elec.candidates);
      const votes = JSON.parse(elec.votes || '{}');

      if (votes[userId]) return interaction.reply({ content: '❌ Vous avez déjà voté dans cette élection.', ephemeral: true });
      ...
      if (!candidat) {
        return interaction.reply({ content: `❌ Candidat "${choix}" introuvable. Disponibles : ${candidats.join(', ')}`, ephemeral: true });
      }
      ...
      return interaction.reply({ content: `✅ Vote enregistré pour **${candidat}** dans l\'élection "${elec.title}" ! (+10 🪙)`, ephemeral: true });
    }
    ...
    if (sub === 'liste') {
      ...
      if (!elections.length) return interaction.reply({ content: '📋 Aucune élection active.', ephemeral: true });
    ...
    if (sub === 'referendum') {
      ...
      const msg = await interaction.reply({ embeds: [new EmbedBuilder().setColor('#E74C3C')
        .setTitle('📊 RÉFÉRENDUM')
