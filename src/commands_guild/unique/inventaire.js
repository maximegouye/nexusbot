...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'voir') {
      ...
      if (!items.length) return interaction.reply({ content: `🎒 **${target.username}** a un inventaire vide. Ouvrez des lootboxes avec \`/inventaire lootbox\` !`, ephemeral: true });
      ...
      return interaction.reply({ embeds: [new EmbedBuilder().setColor('#8E44AD')
    ...
    if (sub === 'item') {
      ...
      return interaction.reply({ embeds: [new EmbedBuilder().setColor(RARITY_COLORS[item.rarity] || '#95A5A6')
    ...
    if (sub === 'donner') {
      ...
      if (!owned || owned.quantity < qty) return interaction.reply({ content: `❌ Vous n\'avez pas assez de cet item (vous en avez ${owned?.quantity || 0}).`, ephemeral: true });
    ...
    if (sub === 'lootbox') {
      ...
      return interaction.reply({ embeds: [new EmbedBuilder().setColor(hasRare ? '#F1C40F' : '#3498DB')
