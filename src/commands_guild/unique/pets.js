...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'adopter') {
      ...
      if (existing) return interaction.reply({ content: '❌ Vous avez déjà un animal ! Utilisez `/pet voir` ou `/pet abandonner`.', ephemeral: true });
      ...
      if ((u.balance||0) < cost) return interaction.reply({ content: `❌ Adopter un **${type}** (${petData.rarity}) coûte **${cost} ${coin}**. Votre solde : ${u.balance||0} ${coin}`, ephemeral: true });
    ...
    if (sub === 'voir') {
      ...
      if (!pet) return interaction.reply({ content: `❌ **${target.username}** n'a pas d'animal. Adoptez-en un avec \`/pet adopter\` !`, ephemeral: true });
    ...
    if (sub === 'nourrir') {
      ...
      if (!pet) return interaction.reply({ content: '❌ Vous n\'avez pas d\'animal. `/pet adopter`', ephemeral: true });

      const foodKey = interaction.options.getString('nourriture');
      const food = FOODS[foodKey];
      const u = db.getUser(userId, guildId);
      if ((u.balance||0) < food.price) return interaction.reply({ content: `❌ **${foodKey}** coûte **${food.price} ${coin}**.`, ephemeral: true });

      const cdLeft = 300 - (now - (pet.last_fed||0));
      if (cdLeft > 0) return interaction.reply({ content: `⏳ Votre animal n'a pas encore faim ! Attendez **${Math.ceil(cdLeft/60)} min**.`, ephemeral: true });
    ...
    if (sub === 'jouer') {
      const pet = db.db.prepare('SELECT * FROM pets WHERE guild_id=? AND user_id=?').get(guildId, userId);
      if (!pet) return interaction.reply({ content: '❌ Vous n\'avez pas d\'animal.', ephemeral: true });
      const cdLeft = 1800 - (now - (pet.last_played||0));
      if (cdLeft > 0) return interaction.reply({ content: `⏳ Votre animal est fatigué ! Revenez dans **${Math.ceil(cdLeft/60)} min**.`, ephemeral: true });
      ...
      return interaction.reply({ embeds: [new EmbedBuilder().setColor('#F1C40F').setTitle(`🎾 ${pet.name} a ${act} !`).addFields({name:'😊 Bonheur',value:`+${happyGain}% → ${newHappy}%`,inline:true},{name:'✨ XP',value:`+${xpGain}`,inline:true})] });
    }

    if (sub === 'entrainer') {
      const pet = db.db.prepare('SELECT * FROM pets WHERE guild_id=? AND user_id=?').get(guildId, userId);
      if (!pet) return interaction.reply({ content: '❌ Pas d\'animal.', ephemeral: true });
      const cdLeft = 3600 - (now - (pet.last_battle||0));
      if (cdLeft > 0) return interaction.reply({ content: `⏳ Votre animal est épuisé ! Revenez dans **${Math.ceil(cdLeft/60)} min**.`, ephemeral: true });
      ...
      return interaction.reply({ embeds: [new EmbedBuilder().setColor('#E74C3C').setTitle(`⚔️ ${pet.name} s'est entraîné !`).addFields({name:'⚔️ ATK',value:`+${atkGain}`,inline:true},{name:'🛡️ DEF',value:`+${defGain}`,inline:true},{name:'✨ XP',value:`+${xpGain}`,inline:true})] });
    }

    if (sub === 'combat') {
      const opponent = interaction.options.getUser('adversaire');
      if (opponent.id === userId) return interaction.reply({ content: '❌ Vous ne pouvez pas vous battre contre vous-même.', ephemeral: true });
      ...
      if (!myPet) return interaction.reply({ content: '❌ Vous n\'avez pas d\'animal.', ephemeral: true });
      if (!theirPet) return interaction.reply({ content: `❌ <@${opponent.id}> n'a pas d'animal.`, ephemeral: true });
    ...
    if (sub === 'evoluer') {
      ...
      if (pet.evolution_stage >= maxStage) return interaction.reply({ content: '✨ Votre animal est déjà à son évolution maximale !', ephemeral: true });
      const levelReq = (pet.evolution_stage + 1) * 10;
      if (pet.level < levelReq) return interaction.reply({ content: `❌ Niveau **${levelReq}** requis pour évoluer. Vous êtes niveau **${pet.level}**.`, ephemeral: true });
    ...
    if (sub === 'top') {
      ...
      if (!top.length) return interaction.reply({ content: '❌ Aucun animal sur ce serveur.', ephemeral: true });
      const desc = top.map((p, i) => `${['🥇','🥈','🥉'][i]||`**${i+1}.**`} ${p.emoji} **${p.name}** (<@${p.user_id}>) — Niv.**${p.level}** • ${p.wins}W/${p.losses}L • ${p.rarity}`).join('\n');
      return interaction.reply({ embeds: [new EmbedBuilder().setColor('#F1C40F').setTitle('🏆 Top Animaux').setDescription(desc)] });
    }
    ...
    if (sub === 'abandonner') {
      ...
      return interaction.reply({ embeds: [new EmbedBuilder().setColor('#E74C3C').setTitle('⚠️ Abandon').setDescription(`Êtes-vous sûr de vouloir abandonner **${pet.name}** ? Cette action est **irréversible**.`)], components: [row], ephemeral: true });
    }
  }
};
