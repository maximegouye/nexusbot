...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'creer') {
      ...
      if (existing) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Vous appartenez déjà à une guilde. Quittez-la d\'abord avec `/guilde quitter`.', ephemeral: true });
      ...
      if ((u.balance || 0) < cost) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: `❌ Créer une guilde coûte **${cost} ${coin}**.`, ephemeral: true });

      const existingName = db.db.prepare('SELECT id FROM syndicats WHERE guild_id=? AND name=?').get(guildId, nom);
      if (existingName) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Une guilde avec ce nom existe déjà.', ephemeral: true });
    ...
    if (sub === 'rejoindre') {
      const existing = db.db.prepare('SELECT id FROM syndicat_members WHERE guild_id=? AND user_id=?').get(guildId, userId);
      if (existing) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Vous êtes déjà dans une guilde.', ephemeral: true });

      const nom = interaction.options.getString('nom');
      const synd = db.db.prepare('SELECT * FROM syndicats WHERE guild_id=? AND name=?').get(guildId, nom);
      if (!synd) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: `❌ Guilde "${nom}" introuvable.`, ephemeral: true });

      const members = JSON.parse(synd.members || '[]');
      if (members.length >= 30) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Cette guilde est complète (30 membres max).', ephemeral: true });
    ...
    if (sub === 'quitter') {
      ...
      if (!m) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Vous n\'êtes dans aucune guilde.', ephemeral: true });

      const synd = db.db.prepare('SELECT * FROM syndicats WHERE id=?').get(m.syndicat_id);
      if (synd.leader_id === userId) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Vous êtes le leader. Transférez le leadership avant de quitter.', ephemeral: true });
      ...
      return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: `✅ Vous avez quitté la guilde **[${synd.tag}] ${synd.name}**.`, ephemeral: true });
    }
    ...
    if (sub === 'info') {
      ...
      if (!synd) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Guilde introuvable.', ephemeral: true });
    ...
    if (sub === 'liste') {
      ...
      if (!synds.length) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Aucune guilde sur ce serveur. Créez-en une avec `/guilde creer` !', ephemeral: true });
      const medals = ['🥇','🥈','🥉'];
    ...
    if (sub === 'don') {
      ...
      const u = db.getUser(userId, guildId);
      if ((u.balance || 0) < montant) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: `❌ Solde insuffisant.`, ephemeral: true });
    ...
    if (sub === 'guerre') {
      ...
      if (synd1.leader_id !== userId) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Seul le leader peut déclarer la guerre.', ephemeral: true });
      ...
      if (!synd2) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: `❌ Guilde "${ennemiNom}" introuvable.`, ephemeral: true });
      if (synd2.id === m.syndicat_id) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Vous ne pouvez pas vous battre contre vous-même !', ephemeral: true });
    ...
    if (sub === 'classement') {
      ...
      if (!synds.length) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Aucune guilde sur ce serveur.', ephemeral: true });
      const medals = ['🥇','🥈','🥉'];
    ...
    if (sub === 'expulser' || sub === 'promouvoir') {
      const m = db.db.prepare('SELECT * FROM syndicat_members WHERE guild_id=? AND user_id=?').get(guildId, userId);
      if (!m || !['leader','officier'].includes(m.role)) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Vous devez être leader ou officier.', ephemeral: true });

      const target = interaction.options.getUser('membre');
      const tm = db.db.prepare('SELECT * FROM syndicat_members WHERE guild_id=? AND user_id=?').get(guildId, target.id);
      if (!tm || tm.syndicat_id !== m.syndicat_id) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Ce membre n\'est pas dans votre guilde.', ephemeral: true });

      const synd = db.db.prepare('SELECT * FROM syndicats WHERE id=?').get(m.syndicat_id);

      if (sub === 'expulser') {
        if (synd.leader_id === target.id) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Vous ne pouvez pas expulser le leader.', ephemeral: true });
        ...
        db.db.prepare('UPDATE syndicats SET members=? WHERE id=?').run(JSON.stringify(members), synd.id);
        return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: `✅ **${target.username}** a été expulsé de la guilde **[${synd.tag}] ${synd.name}**.` });
      } else {
        if (synd.leader_id !== userId) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Seul le leader peut promouvoir.', ephemeral: true });
        ...
        return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: `⭐ **${target.username}** est maintenant **officier** de **[${synd.tag}] ${synd.name}** !` });
      }
    }
  }
};
