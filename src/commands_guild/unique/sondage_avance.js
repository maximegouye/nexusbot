...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'creer') {
      ...
      if (options.length < 2) return interaction.reply({ content: '❌ Minimum 2 options séparées par `|`.', ephemeral: true });
      if (options.length > 10) return interaction.reply({ content: '❌ Maximum 10 options.', ephemeral: true });
    ...
    if (sub === 'terminer' || sub === 'resultats') {
      ...
      if (!sondage) return interaction.reply({ content: '❌ Sondage introuvable.', ephemeral: true });

      if (sub === 'terminer') {
        if (sondage.user_id !== userId && !interaction.member.permissions.has(0x4000n))
          return interaction.reply({ content: '❌ Seul le créateur ou un modérateur peut terminer ce sondage.', ephemeral: true });
