...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (!interaction.member.permissions.has(0x20n)) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Admin uniquement.', ephemeral: true });
    ...
    if (sub === 'setup') {
      ...
      try {
        ...
        return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: `❌ Impossible de créer le salon : ${e.message}`, ephemeral: true });
      }
    ...
    if (sub === 'update') {
      ...
      if (!counters.length) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Aucun compteur configuré.', ephemeral: true });
      ...
      return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: `✅ **${updated}** compteur(s) mis à jour.`, ephemeral: true });
    }

    if (sub === 'supprimer') {
      const type = interaction.options.getString('type');
      const counter = db.db.prepare("SELECT * FROM compteurs WHERE guild_id=? AND type LIKE ?").get(guildId, `${type}:%`);
      if (!counter) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Compteur introuvable.', ephemeral: true });
      ...
      return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '✅ Compteur supprimé.', ephemeral: true });
    }
  }
};
