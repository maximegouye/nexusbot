...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(s => s.setName('acheter').setDescription('🛒 Acheter une carte sur le marché')
      .addIntegerOption(o => o.setName('id_annonce').setDescription('ID de l\'annonce').setRequired(true)))
  ...
  async execute(interaction) {
    ...
    if (sub === 'ouvrir') {
      ...
      if ((u.balance||0) < cost) return interaction.reply({ content: `❌ Ce pack coûte **${cost} ${coin}**.`, ephemeral: true });
    ...
    if (sub === 'collection') {
      ...
      if (!cards.length) return interaction.reply({ content: `❌ **${target.username}** n'a aucune carte. Ouvrez des packs avec \`/cartes ouvrir\` !`, ephemeral: true });
    ...
    if (sub === 'vendre') {
      ...
      return interaction.reply({ content: `✅ **${cardData?.name || cardId}** mise en vente pour **${prix} ${coin}** !` });
    }
    ...
    if (sub === 'marche') {
      ...
      if (!listings.length) return interaction.reply({ content: '❌ Aucune carte en vente en ce moment.', ephemeral: true });
      ...
      return interaction.reply({ embeds: [new EmbedBuilder().setColor('#F1C40F').setTitle('🏪 Marché des Cartes').setDescription(desc)] });
    }
    ...
    if (sub === 'acheter') {
      ...
      const u = db.getUser(userId, guildId);
      if ((u.balance||0) < listing.price) return interaction.reply({ content: `❌ Vous avez besoin de **${listing.price} ${coin}**.`, ephemeral: true });
      ...
      return interaction.reply({ embeds: [new EmbedBuilder().setColor('#2ECC71').setTitle('✅ Achat réussi !').setDescription(`Vous avez acheté **${card?.name||listing.card_id}** pour **${listing.price} ${coin}** !`)] });
    }

    if (sub === 'duel') {
      const opponent = interaction.options.getUser('adversaire');
      const myCardId = interaction.options.getString('votre_carte');
      if (opponent.id === userId) return interaction.reply({ content: '❌ Vous ne pouvez pas vous défier vous-même.', ephemeral: true });

      const myCard = CARTES.find(c => c.id === myCardId);
      if (!myCard) return interaction.reply({ content: `❌ Carte \`${myCardId}\` introuvable.`, ephemeral: true });

      const owned = db.db.prepare('SELECT * FROM user_cards WHERE guild_id=? AND user_id=? AND card_id=?').get(guildId, userId, myCardId);
      if (!owned) return interaction.reply({ content: '❌ Vous ne possédez pas cette carte.', ephemeral: true });
      ...
      if (!theirCards.length) return interaction.reply({ content: `❌ <@${opponent.id}> n'a aucune carte.`, ephemeral: true });

      const theirBest = theirCards.map(r => CARTES.find(c => c.id === r.card_id)).filter(Boolean).sort((a,b) => (b.atk+b.def) - (a.atk+a.def))[0];
      if (!theirBest) return interaction.reply({ content: '❌ Impossible de trouver la carte adverse.', ephemeral: true });
