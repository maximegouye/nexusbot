...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'ajouter') {
      ...
      if (existing) return interaction.reply({ content: `❌ Une auto-réponse pour **"${trigger}"** existe déjà (#${existing.id}). Supprimez-la d'abord.`, ephemeral: true });
    ...
    if (sub === 'supprimer') {
      ...
      if (!entry) return interaction.reply({ content: `❌ Auto-réponse #${id} introuvable.`, ephemeral: true });
    ...
    if (sub === 'liste') {
      ...
      if (!entries.length) return interaction.reply({ content: '💬 Aucune auto-réponse. Créez-en avec `/autorespond ajouter`.', ephemeral: true });
    ...
    if (sub === 'voir') {
      const id = parseInt(interaction.options.getString('id'));
      const entry = db.db.prepare('SELECT * FROM auto_responses WHERE id=? AND guild_id=?').get(id, guildId);
      if (!entry) return interaction.reply({ content: `❌ #${id} introuvable.`, ephemeral: true });
    ...
    if (sub === 'activer') {
      ...
      return interaction.reply({ embeds: [new EmbedBuilder().setColor(actif ? '#2ECC71' : '#95A5A6')
