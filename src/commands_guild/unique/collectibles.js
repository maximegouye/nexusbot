...
module.exports={
  ...
  async execute(interaction){
    ...
    if(sub==='collection'){
      ...
      return interaction.reply({embeds:[new EmbedBuilder().setColor('#9B59B6').setTitle(`🃏 Collection de ${t.username}`).setThumbnail(t.displayAvatarURL()).addFields(fields).setFooter({text:`${cards.length}/${LIST.length} cartes`})],ephemeral:true});
    }
    if(sub==='infos'){
      const cid=interaction.options.getString('carte').toLowerCase(); const card=CARDS[cid];
      if(!card)return interaction.reply({content:`❌ Carte \`${cid}\` introuvable. IDs: ${LIST.map(c=>c.id).join(', ')}`,ephemeral:true});
      const h=db.db.prepare('SELECT COUNT(*) as cnt FROM collectibles WHERE guild_id=? AND card_id=?').get(guildId,cid).cnt;
      return interaction.reply({embeds:[new EmbedBuilder().setColor(card.color).setTitle(card.name).addFields({name:'🎖️ Rareté',value:`**${card.rarity}**`,inline:true},{name:'🆔 ID',value:`\`${card.id}\``,inline:true},{name:'👥 Détenteurs',value:`**${h}**`,inline:true})]});
    }
    if(sub==='echanger'){
      const target=interaction.options.getUser('membre'); const cid=interaction.options.getString('carte').toLowerCase();
      if(target.id===userId)return interaction.reply({content:'❌ Impossible.',ephemeral:true});
      const card=CARDS[cid]; if(!card)return interaction.reply({content:`❌ Carte introuvable.`,ephemeral:true});
      const owned=getUserCards(userId,guildId); if(!owned.includes(cid))return interaction.reply({content:`❌ Vous ne possédez pas \`${cid}\`.`,ephemeral:true});
      ...
      return interaction.reply({embeds:[new EmbedBuilder().setColor('#2ECC71').setTitle('🤝 Échange !').setDescription(`**${card.name}** (${card.rarity}) offert à ${target}.`)]});
    }
  }
};
