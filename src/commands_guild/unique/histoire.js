...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'creer') {
      ...
      if (active.c >= 5) return interaction.reply({ content: '❌ Maximum 5 histoires actives par serveur. Terminez-en une d\'abord.', ephemeral: true });
    ...
    if (sub === 'ecrire') {
      ...
      if (story.status !== 'active') return interaction.reply({ content: '❌ Cette histoire est terminée.', ephemeral: true });
      ...
      if (content.length > 0 && content[content.length - 1].user_id === userId) {
        return interaction.reply({ content: '❌ Vous avez déjà écrit la dernière phrase. Attendez qu\'un autre membre contribue !', ephemeral: true });
      }

      if (content.length >= story.max_length) {
        db.db.prepare('UPDATE histoires SET status=? WHERE id=?').run('terminee', id);
        return interaction.reply({ content: `📖 L'histoire #${id} a atteint sa longueur maximale et est maintenant **terminée** !` });
      }
    ...
    if (sub === 'lire') {
      ...
      if (page >= totalPages) return interaction.reply({ content: `❌ Page invalide (max: ${totalPages}).`, ephemeral: true });
    ...
    if (sub === 'liste') {
      ...
      if (!stories.length) return interaction.reply({ content: '📚 Aucune histoire active pour l\'instant. Créez-en une avec `/histoire creer` !', ephemeral: true });
    ...
    if (sub === 'terminer') {
      ...
      if (story.creator_id !== userId && !interaction.member.permissions.has(0x8n)) {
        return interaction.reply({ content: '❌ Seul le créateur ou un administrateur peut terminer cette histoire.', ephemeral: true });
      }
      db.db.prepare('UPDATE histoires SET status=? WHERE id=?').run('terminee', id);
      const content = JSON.parse(story.content || '[]');
