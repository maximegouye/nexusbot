...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(s => s.setName('supprimer_question').setDescription('🗑️ Supprimer une question (Admin)')
      .addStringOption(o => o.setName('formulaire').setDescription('Nom du formulaire').setRequired(true))
      .addIntegerOption(o => o.setName('numero').setDescription('Numéro de la question').setRequired(true).setMinValue(1)))
  ...
  async execute(interaction) {
    ...
    if (sub === 'creer') {
      if (!isAdmin) return interaction.reply({ content: '❌ Admin uniquement.', ephemeral: true });
    ...
    if (sub === 'question') {
      ...
      if (!form) return interaction.reply({ content: `❌ Formulaire \`${nom}\` introuvable.`, ephemeral: true });
      ...
      if (questions.length >= 5) return interaction.reply({ content: '❌ Maximum 5 questions par formulaire.', ephemeral: true });
    ...
    if (sub === 'supprimer_question') {
      ...
      const questions = JSON.parse(form.questions || '[]');
      if (num < 0 || num >= questions.length) return interaction.reply({ content: '❌ Numéro invalide.', ephemeral: true });
    ...
    if (sub === 'liste') {
      ...
      if (!forms.length) return interaction.reply({ content: '❌ Aucun formulaire sur ce serveur.', ephemeral: true });
    ...
    if (sub === 'voir') {
      ...
      if (!subs.length) return interaction.reply({ content: '❌ Vous n\'avez aucune candidature.', ephemeral: true });
...
async function handleApplication(interaction, guildId, userId, nom) {
  ...
  const form = db.db.prepare('SELECT * FROM app_forms WHERE guild_id=? AND name=?').get(guildId, nom);
  if (!form) return interaction.reply({ content: `❌ Formulaire \`${nom}\` introuvable.`, ephemeral: true });
  if (form.status === 'closed') return interaction.reply({ content: '❌ Ce formulaire est actuellement fermé.', ephemeral: true });

  const questions = JSON.parse(form.questions || '[]');
  if (!questions.length) return interaction.reply({ content: '❌ Ce formulaire n\'a pas encore de questions.', ephemeral: true });
  ...
  if (existing) return interaction.reply({ content: '❌ Vous avez déjà une candidature en attente pour ce formulaire.', ephemeral: true });
