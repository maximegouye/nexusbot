...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'voir') {
      ...
      if (!notes.length) {
        if (interaction.deferred || interaction.replied) {
          return interaction.editReply({ embeds: [new EmbedBuilder().setColor('#95a5a6').setDescription(`Aucune note pour **${target.username}**.`)] });
        } else {
          return interaction.reply({ embeds: [new EmbedBuilder().setColor('#95a5a6').setDescription(`Aucune note pour **${target.username}**.`)] });
        }
      }
      const embed = new EmbedBuilder()
        .setColor('#f39c12')
    ...
    if (sub === 'supprimer') {
      ...
      if (!note) {
        if (interaction.deferred || interaction.replied) {
          return interaction.editReply({ embeds: [new EmbedBuilder().setColor('#e74c3c').setDescription(`❌ Note #${noteId} introuvable.`)] });
        } else {
          return interaction.reply({ embeds: [new EmbedBuilder().setColor('#e74c3c').setDescription(`❌ Note #${noteId} introuvable.`)] });
        }
      }
      ...
      if (interaction.deferred || interaction.replied) {
        ...
        return interaction.reply({ embeds: [new EmbedBuilder().setColor('#2ecc71').setDescription(`✅ Note **#${noteId}** supprimée.`)] });
      }
    }
    ...
    if (sub === 'recents') {
      ...
      if (!notes.length) {
        if (interaction.deferred || interaction.replied) {
          return interaction.editReply({ embeds: [new EmbedBuilder().setColor('#95a5a6').setDescription('Aucune note récente.')] });
        } else {
          return interaction.reply({ embeds: [new EmbedBuilder().setColor('#95a5a6').setDescription('Aucune note récente.')] });
        }
      }
      const embed = new EmbedBuilder()
        .setColor('#f39c12')
