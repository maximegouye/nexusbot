  ...
  async execute(interaction) {
    ...
    if (sub === 'liste') {
      try {
        ...
        if (!sounds.size) {
          return interaction.reply({ embeds: [ef.info('🔊 Soundboard', 'Ce serveur n\'a aucun son dans sa soundboard pour l\'instant.\n\nAjoutez-en via **Paramètres du serveur → Soundboard**.')], ephemeral: true });
        }
        ...
        return interaction.reply({ embeds: [ef.error('Erreur', `Impossible de lire la soundboard : ${e.message}`)], ephemeral: true });
      }
    }
    ...
    if (sub === 'jouer') {
      ...
      if (!voice) {
        return interaction.reply({ embeds: [ef.warning('Rejoins un vocal', 'Tu dois être dans un salon vocal pour que je puisse jouer un son.')], ephemeral: true });
      }
      // Permissions
      const perms = voice.permissionsFor(interaction.client.user);
      if (!perms?.has(PermissionFlagsBits.Connect) || !perms?.has(PermissionFlagsBits.Speak)) {
        return interaction.reply({ embeds: [ef.error('Permissions manquantes', 'Le bot doit pouvoir **se connecter** et **parler** dans ce salon vocal.')], ephemeral: true });
      }
      ...
      try {
        ...
        if (!sound) {
          return interaction.reply({ embeds: [ef.error('Son introuvable', `Aucun son nommé \`${name}\`. Utilise \`/son liste\` pour voir les sons disponibles.`)], ephemeral: true });
        }
