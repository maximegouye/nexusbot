...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (!isAdmin(interaction.member)) {
      return interaction.reply({ content: '❌ Vous devez être **administrateur** pour utiliser ce panneau.', ephemeral: true });
    }
    ...
    if (group === 'monnaie') {
      ...
      if (sub === 'donner') {
        ...
        if (bl) return interaction.reply({ content: `❌ **${target.username}** est blacklisté de l\'économie.`, ephemeral: true });
      ...
      if (sub === 'reset_total') {
        ...
        if (confirm !== 'CONFIRMER') return interaction.reply({ content: '❌ Vous devez taper exactement **CONFIRMER** pour valider cette action dangereuse.', ephemeral: true });
    ...
    if (group === 'xp') {
      ...
      if (sub === 'reset_serveur') {
        const confirm = interaction.options.getString('confirmation');
        if (confirm !== 'CONFIRMER') return interaction.reply({ content: '❌ Tapez exactement **CONFIRMER**.', ephemeral: true });
        const count = db.db.prepare('SELECT COUNT(*) as c FROM users WHERE guild_id=?').get(guildId);
    ...
    if (group === 'utilisateur') {
      ...
      if (sub === 'info') {
        ...
        return interaction.reply({ embeds: [new EmbedBuilder().setColor('#7C3AED')
    ...
    if (group === 'serveur') {
      ...
      if (sub === 'activite') {
        ...
        return interaction.reply({ embeds: [new EmbedBuilder().setColor('#10B981')
    ...
    if (group === 'items') {
      ...
      if (sub === 'donner') {
        ...
        if (!shopItem) return interaction.reply({ content: `❌ Item #${item_id} introuvable dans la boutique.`, ephemeral: true });
      ...
      if (sub === 'voir') {
        ...
        if (!items.length) return interaction.reply({ content: `🎒 <@${target.id}> a un inventaire vide.`, ephemeral: true });
        const lines = items.slice(0, 20).map(it => `${it.emoji || '📦'} **${it.name || `Item #${it.item_id}`}** ×${it.quantity}`).join('\n');
        return interaction.reply({ embeds: [new EmbedBuilder().setColor('#8B5CF6')
