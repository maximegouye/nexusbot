...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'acheter') {
      ...
      if (!marche[sym]) return interaction.reply({ content: `❌ Action **${sym}** introuvable.`, ephemeral: true });
      ...
      if (!row || row.coins < total) return interaction.reply({ content: `❌ Fonds insuffisants. Besoin: **${total.toLocaleString()} 🪙**, Solde: **${(row?.coins || 0).toLocaleString()} 🪙**.`, ephemeral: true });
    ...
    if (sub === 'vendre') {
      ...
      if (isNaN(qty) || qty <= 0) return interaction.reply({ content: '❌ Quantité invalide.', ephemeral: true });
      const existing = await db.get('SELECT quantite FROM portfolio WHERE userId = ? AND guildId = ? AND symbole = ?', [userId, guildId, sym]);
      if (!existing || existing.quantite < qty) return interaction.reply({ content: `❌ Pas assez d'actions **${sym}**.`, ephemeral: true });
      const total = marche[sym].prix * qty;
      await db.run('UPDATE economy SET coins = coins + ? WHERE userId = ? AND guildId = ?', [total, userId, guildId]);
    ...
    if (sub === 'portefeuille') {
      ...
      if (!rows || rows.length === 0) return interaction.reply({ content: `📭 **${target.username}** n'a aucune action.`, ephemeral: true });
    ...
    if (sub === 'classement') {
      ...
      if (!rows || rows.length === 0) return interaction.reply({ content: '📭 Aucun investisseur pour le moment.', ephemeral: true });
