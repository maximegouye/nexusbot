...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'creer') {
      ...
      if (existing) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Vous avez déjà une entreprise !', ephemeral: true });
    ...
    if (sub === 'voir') {
      ...
      if (!ent) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: `❌ **${target.username}** n'a pas d'entreprise.`, ephemeral: true });
      ...
      return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ embeds: [new EmbedBuilder().setColor('#3498DB').setTitle(`${s.emoji} ${ent.name}`)
    ...
    if (sub === 'collecter') {
      ...
      if (!ent) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Vous n\'avez pas d\'entreprise.', ephemeral: true });
      ...
      return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ embeds: [new EmbedBuilder().setColor('#2ECC71').setTitle(`💰 Revenus collectés — ${ent.name}`)
    ...
    if (sub === 'embaucher') {
      ...
      const employees = JSON.parse(ent.employees || '[]');
      if (employees.length >= 10) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Maximum 10 employés.', ephemeral: true });
      if (employees.includes(target.id)) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Ce membre est déjà employé.', ephemeral: true });

      const salaryCost = 500;
      const u = db.getUser(userId, guildId);
      if ((u.balance||0) < salaryCost) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: `❌ L'embauche coûte **${salaryCost} ${coin}**.`, ephemeral: true });
      ...
      return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: `✅ <@${target.id}> embauché(e) ! Votre entreprise génère maintenant **+5% de revenus** en plus. (${employees.length}/10 employés)` });
    }
    ...
    if (sub === 'licencier') {
      ...
      const target = interaction.options.getUser('membre');
      const employees = JSON.parse(ent.employees || '[]');
      if (!employees.includes(target.id)) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Ce membre n\'est pas votre employé.', ephemeral: true });
      ...
      return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: `✅ <@${target.id}> a été licencié(e). (${newEmp.length}/10 employés)` });
    }

    if (sub === 'top') {
      const top = db.db.prepare('SELECT * FROM entreprises WHERE guild_id=? ORDER BY treasury DESC LIMIT 10').all(guildId);
      if (!top.length) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Aucune entreprise sur ce serveur.', ephemeral: true });
