...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'collecter') {
      ...
      if (cd > 0) return interaction.reply({ content: `⏳ Réessayez dans **${cd}s**.`, ephemeral: true });
    ...
    if (sub === 'fabriquer') {
      ...
      for (const [r, q] of Object.entries(recette.ingredients)) {
        if ((res[r] || 0) < q) {
          return interaction.reply({ content: `❌ Ressources insuffisantes ! Vous manquez de **${RESOURCE_EMOJIS[r]} ${r}** (besoin: ${q}, avez: ${res[r] || 0}).`, ephemeral: true });
        }
      }
