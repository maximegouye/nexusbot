...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'rembourser') {
      ...
      if (!pret) return interaction.reply({ content: '✅ Vous n\'avez aucun prêt actif.', ephemeral: true });
      ...
      if (u.balance < toRepay) return interaction.reply({ content: `❌ Solde insuffisant. Vous devez **${toRepay} ${coin}**${penaltyMsg}.`, ephemeral: true });
    ...
    if (sub === 'statut') {
      const pret = db.db.prepare("SELECT * FROM prets WHERE guild_id=? AND user_id=? AND repaid=0").get(guildId, userId);
      if (!pret) return interaction.reply({ content: '✅ Aucun prêt actif.', ephemeral: true });
