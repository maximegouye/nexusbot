...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(s => s.setName('acheter').setDescription('💸 Acheter une offre')
      .addIntegerOption(o => o.setName('numero').setDescription('Numéro de l\'offre (1-4)').setRequired(true).setMinValue(1).setMaxValue(4)))
  ...
  async execute(interaction) {
    ...
    if (sub === 'acheter') {
      ...
      if ((u.balance || 0) < item.price) {
        return interaction.reply({ content: `❌ Pas assez de ${coin}. Il vous faut **${item.price.toLocaleString()}**, vous avez **${u.balance || 0}**.`, ephemeral: true });
      }
    ...
    if (sub === 'inventaire') {
      ...
      if (!fields.length) return interaction.reply({ content: '❌ Votre inventaire spécial est vide.', ephemeral: true });
