...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'voir') {
      ...
      if (!items.length) return interaction.reply({ content: '❌ La boutique est vide. Un admin peut ajouter des articles avec `/boutique ajouter`.', ephemeral: true });
    ...
    if (sub === 'acheter') {
      ...
      if (!item) return interaction.reply({ content: `❌ Article **${nom}** introuvable. Voir la boutique : \`/boutique voir\`.`, ephemeral: true });
      if (item.stock === 0) return interaction.reply({ content: `❌ **${item.name}** est épuisé !`, ephemeral: true });

      const u = db.getUser(userId, guildId);
      if (u.balance < item.price) return interaction.reply({ content: `❌ Solde insuffisant. **${item.price} ${coin}** requis, vous avez **${u.balance} ${coin}**.`, ephemeral: true });
      ...
      if (existing && item.role_id) return interaction.reply({ content: `❌ Vous possédez déjà **${item.name}** !`, ephemeral: true });
    ...
    if (sub === 'inventaire') {
      ...
      if (!inv.length) return interaction.reply({ content: `❌ ${target.id === userId ? 'Votre inventaire est vide.' : `<@${target.id}> n'a rien dans son inventaire.`}`, ephemeral: true });
    ...
    if (sub === 'utiliser') {
      ...
      if (!invItem) return interaction.reply({ content: `❌ Vous ne possédez pas **${nom}** ou votre stock est épuisé.`, ephemeral: true });
    ...
    if (sub === 'ajouter') {
      ...
      try {
        ...
        return interaction.reply({ content: `❌ Un article avec ce nom existe déjà.`, ephemeral: true });
      }
    ...
    if (sub === 'supprimer') {
      ...
      if (!r.changes) return interaction.reply({ content: `❌ Article **${nom}** introuvable.`, ephemeral: true });
      return interaction.reply({ content: `✅ Article **${nom}** supprimé.`, ephemeral: true });
    }
  }
};
