...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'placer') {
      ...
      if (u.balance < montant) return interaction.reply({ content: `❌ Solde insuffisant (**${u.balance} ${coin}**).`, ephemeral: true });
      ...
      if (active.c >= 5) return interaction.reply({ content: '❌ Maximum 5 investissements simultanés.', ephemeral: true });
    ...
    if (sub === 'portefeuille') {
      ...
      if (!invs.length) return interaction.reply({ content: '📈 Aucun investissement actif.', ephemeral: true });
    ...
    if (sub === 'collecter') {
      ...
      if (!ready.length) return interaction.reply({ content: '⏳ Aucun investissement arrivé à maturité.', ephemeral: true });
