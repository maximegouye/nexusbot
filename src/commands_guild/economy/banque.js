...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if(sub==='retirer'){
      ...
      return interaction.reply({embeds:[new EmbedBuilder().setColor('#E74C3C').setTitle('📤 Retrait effectué').addFields({name:'💰 Retiré',value:`**-${fmt(m)} ${coin}**`,inline:true},{name:'🏦 Banque',value:`**${fmt(bank-m)} ${coin}**`,inline:true})]});
    }
    if(sub==='interets'){
      ...
      if(bank===0) return interaction.reply({content:'❌ Aucun argent en banque.',ephemeral:true});
    ...
    if(sub==='historique'){
      ...
      if(!txs.length) return interaction.reply({content:'📋 Aucune transaction.',ephemeral:true});
    ...
    if(sub==='virer'){
      ...
      return interaction.reply({embeds:[new EmbedBuilder().setColor('#2ECC71').setTitle('💸 Virement effectué').setDescription(`**${fmt(m)} ${coin}** viré à ${target}.`)]});
    }
    if(sub==='tiers'){
      ...
      return interaction.reply({embeds:[new EmbedBuilder().setColor('#FFD700').setTitle('🏆 Tiers VIP Bancaires').setDescription(`Votre tier: ${tier.emoji} **${tier.name}** — intérêts **${tier.interest*100}%/jour**`).addFields(fields).setFooter({text:'Plus vous déposez, plus vous gagnez !'})],ephemeral:true});
    }
  }
};
