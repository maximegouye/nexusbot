...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'monter') {
      ...
      if (!nextPData) return interaction.reply({ content: '🌟 Vous avez atteint le **prestige maximum** ! Félicitations !', ephemeral: true });

      const currentLevel = u.level || 1;
      if (currentLevel < nextPData.required_xp_level) {
        return interaction.reply({ content: `❌ Vous devez être au niveau **${nextPData.required_xp_level}** minimum.\nActuellement : **${currentLevel}**`, ephemeral: true });
      }
    ...
    if (sub === 'top') {
      ...
      if (!top.length) return interaction.reply({ content: '❌ Aucun membre avec du prestige sur ce serveur.', ephemeral: true });
