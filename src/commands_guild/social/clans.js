...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'creer') {
      ...
      if (existing) return interaction.reply({ content: '❌ Vous êtes déjà dans un clan. Quittez-le d\'abord.', ephemeral: true });
      ...
      if (u.balance < 500) return interaction.reply({ content: `❌ Créer un clan coûte **500 ${coin}**.`, ephemeral: true });
      ...
      try {
        ...
        return interaction.reply({ content: `❌ Ce nom ou tag existe déjà.`, ephemeral: true });
      }
    ...
    if (sub === 'rejoindre') {
      const existing = db.db.prepare('SELECT * FROM clan_members WHERE guild_id=? AND user_id=?').get(guildId, userId);
      if (existing) return interaction.reply({ content: '❌ Vous êtes déjà dans un clan.', ephemeral: true });

      const nom = interaction.options.getString('nom');
      const clan = db.db.prepare('SELECT * FROM clans WHERE guild_id=? AND LOWER(name)=LOWER(?)').get(guildId, nom);
      if (!clan) return interaction.reply({ content: `❌ Clan **${nom}** introuvable.`, ephemeral: true });
      ...
      if (members.c >= maxSize) return interaction.reply({ content: `❌ Ce clan est plein (${members.c}/${maxSize} membres).`, ephemeral: true });
    ...
    if (sub === 'quitter') {
      ...
      if (m.owner_id === userId) return interaction.reply({ content: '❌ Vous êtes chef ! Transférez la direction avant de partir.', ephemeral: true });

      db.db.prepare('DELETE FROM clan_members WHERE guild_id=? AND user_id=?').run(guildId, userId);
      return interaction.reply({ content: `✅ Vous avez quitté le clan **${m.emoji} ${m.name}**.`, ephemeral: true });
    }
    ...
    if (sub === 'kick') {
      ...
      if (!m) return interaction.reply({ content: '❌ Vous n\'êtes dans aucun clan.', ephemeral: true });
      if (m.rank !== 'chef' && m.rank !== 'officier') return interaction.reply({ content: '❌ Chef ou officier uniquement.', ephemeral: true });

      const target = interaction.options.getUser('membre');
      if (target.id === userId) return interaction.reply({ content: '❌ Vous ne pouvez pas vous exclure vous-même.', ephemeral: true });
      ...
      if (!targetM) return interaction.reply({ content: `❌ <@${target.id}> n'est pas dans votre clan.`, ephemeral: true });
      if (targetM.rank === 'chef') return interaction.reply({ content: '❌ Impossible d\'expulser le chef.', ephemeral: true });

      db.db.prepare('DELETE FROM clan_members WHERE guild_id=? AND user_id=?').run(guildId, target.id);
      return interaction.reply({ content: `✅ <@${target.id}> a été exclu du clan.` });
    }
    ...
    if (sub === 'promouvoir') {
      ...
      if (!m || m.rank !== 'chef') return interaction.reply({ content: '❌ Chef uniquement.', ephemeral: true });
      ...
      return interaction.reply({ content: `✅ <@${target.id}> est maintenant **${rankLabels[rang]}** !` });
    }
    ...
    if (sub === 'don') {
      ...
      return interaction.reply({ content: `✅ Vous avez donné **${montant} ${coin}** à la trésorerie du clan !` });
    }
  }
};
