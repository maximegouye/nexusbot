...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'proposer') {
      ...
      if (target.id === userId) return interaction.reply({ content: '❌ Tu ne peux pas te marier avec toi-même !', ephemeral: true });
      ...
      if (existingU1) return interaction.reply({ content: '❌ Tu es déjà marié(e) ! Divorce d\'abord avec `/mariage divorcer`.', ephemeral: true });
      ...
      if (existingU2) return interaction.reply({ content: `❌ <@${target.id}> est déjà marié(e) !`, ephemeral: true });
    ...
    if (sub === 'divorcer') {
      ...
      if (!mariage) return interaction.reply({ content: '❌ Tu n\'es pas marié(e).', ephemeral: true });
    ...
    if (sub === 'liste') {
      ...
      if (!couples.length) return interaction.reply({ content: '❌ Aucun couple sur ce serveur.', ephemeral: true });
