...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'status') {
      ...
      return interaction.reply({ content: `✅ Statut mis à jour : **${texte}**`, ephemeral: true });
    }

    if (sub === 'couleur') {
      const hex = interaction.options.getString('hex').replace('#','');
      if (!/^[0-9A-Fa-f]{6}$/.test(hex)) return interaction.reply({ content: '❌ Code HEX invalide.', ephemeral: true });
      db.db.prepare('UPDATE statuts_perso SET color=? WHERE guild_id=? AND user_id=?').run(`#${hex.toUpperCase()}`, guildId, userId);
      return interaction.reply({ content: `✅ Couleur de profil mise à jour : **#${hex.toUpperCase()}**`, ephemeral: true });
    }
    ...
    if (sub === 'confidentialite') {
      ...
      return interaction.reply({ content: '✅ Paramètres de confidentialité mis à jour.', ephemeral: true });
    }
