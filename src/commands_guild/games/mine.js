...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(s => s.setName('profondeur').setDescription('⬇️ Descendre plus profond dans la mine')
      .addIntegerOption(o => o.setName('niveau').setDescription('Niveau de profondeur désiré').setRequired(true).setMinValue(1).setMaxValue(10)))
  ...
  async execute(interaction) {
    ...
    if (sub === 'profondeur') {
      ...
      if (niveau <= m.depth) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: `❌ Vous êtes déjà à la profondeur **${m.depth}**. Choisissez un niveau plus élevé.`, ephemeral: true });

      let totalCost = 0;
      for (let i = m.depth + 1; i <= niveau; i++) totalCost += depthCosts[i - 1] || 100000;

      const u = db.getUser(userId, guildId);
      if (u.balance < totalCost) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: `❌ Coût total pour atteindre la profondeur ${niveau}: **${totalCost} ${coin}**.`, ephemeral: true });
    ...
    if (sub === 'pioche') {
      ...
      if (m.pickaxe_level >= 9) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '✅ Pioche déjà maximale !', ephemeral: true });
      ...
      return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: `✅ Pioche améliorée → Niv.**${m.pickaxe_level + 1}** (+${m.pickaxe_level * 15}%)`, ephemeral: false });
    }
    ...
    if (sub === 'classement') {
      ...
      if (!top.length) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Aucun mineur.', ephemeral: true });
      const medals = ['🥇', '🥈', '🥉'];
