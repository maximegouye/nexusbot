...
module.exports = {
  data: new SlashCommandBuilder().setName('tournoi').setDescription('🏆 Tournois par élimination')
    .addSubcommand(s=>s.setName('creer').setDescription('🆕 Créer un nouveau tournoi')
      .addStringOption(o=>o.setName('nom').setDescription('Nom du tournoi').setRequired(true).setMaxLength(50))
      .addIntegerOption(o=>o.setName('mise').setDescription("Mise d'entrée (€)").setMinValue(0).setMaxValue(100000))
      .addIntegerOption(o=>o.setName('max').setDescription('Nombre max de joueurs (défaut: 8)').setMinValue(2).setMaxValue(32)))
  ...
  async execute(interaction) {
    ...
    if(sub==='creer'){
      if(eng.getActiveTournament(guildId)) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({content:'❌ Tournoi déjà en cours.',ephemeral:true});
      ...
      return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({embeds:[new EmbedBuilder().setColor('#F1C40F').setTitle(`🏆 Tournoi "${nom}" créé !`).setDescription('Ouvert aux inscriptions ! `/tournoi rejoindre`').addFields({name:'💰 Mise',value:fee>0?`**${fee} ${coin}**`:'Gratuit',inline:true},{name:'👥 Max',value:`**${max}**`,inline:true},{name:'👑 Hôte',value:`${interaction.user}`,inline:true}).setFooter({text:'/tournoi demarrer pour lancer'})]});
    }
    if(sub==='rejoindre'){
      const t=eng.getActiveTournament(guildId);
      if(!t) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({content:'❌ Aucun tournoi ouvert.',ephemeral:true});
      if(t.status!=='open') return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({content:'❌ Tournoi déjà lancé.',ephemeral:true});
      const r=eng.joinTournament(t.id,userId,guildId);
      if(!r.ok) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({content:`❌ ${r.reason}`,ephemeral:true});
      const nb=eng.getAllPlayers(t.id).length;
      return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({embeds:[new EmbedBuilder().setColor('#2ECC71').setTitle(`✅ Inscrit !`).setDescription(`**${nb}/${t.max_players}** joueurs inscrits.`)]});
    }
    if(sub==='demarrer'){
      const t=eng.getActiveTournament(guildId);
      ...
      if(t.status!=='open') return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({content:'❌ Déjà lancé.',ephemeral:true});
      const r=eng.startTournament(t.id); if(!r.ok) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({content:`❌ ${r.reason}`,ephemeral:true});
      ...
      return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({embeds:[new EmbedBuilder().setColor('#E74C3C').setTitle(`⚔️ Tournoi "${t.name}" lancé !`).addFields({name:'🥊 Matchs Round 1',value:list||'—'},{name:'💰 Prize Pool',value:`**${t.prize_pool} ${coin}**`,inline:true}).setFooter({text:'/tournoi jouer pour résoudre'})]});
    }
    if(sub==='jouer'){
      const t=eng.getActiveTournament(guildId);
      if(!t||t.status!=='running'||t.host_id!==userId) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({content:'❌ Pas de tournoi en cours / pas l\'hôte.',ephemeral:true});
      const pending=eng.getPendingMatches(t.id); if(!pending.length) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({content:'❌ Aucun match en attente.',ephemeral:true});
      const results=[];
    ...
    if(sub==='statut'){
      const t=eng.getActiveTournament(guildId); if(!t) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({content:'❌ Aucun tournoi actif.',ephemeral:true});
