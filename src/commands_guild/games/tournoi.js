...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if(sub==='rejoindre'){
      ...
      if(!t) return interaction.reply({content:'❌ Aucun tournoi ouvert.',ephemeral:true});
      if(t.status!=='open') return interaction.reply({content:'❌ Tournoi déjà lancé.',ephemeral:true});
      ...
      return interaction.reply({embeds:[new EmbedBuilder().setColor('#2ECC71').setTitle(`✅ Inscrit !`).setDescription(`**${nb}/${t.max_players}** joueurs inscrits.`)]});
    }
    if(sub==='demarrer'){
      const t=eng.getActiveTournament(guildId);
      if(!t||t.host_id!==userId) return interaction.reply({content:'❌ Pas de tournoi / pas l\'hôte.',ephemeral:true});
      if(t.status!=='open') return interaction.reply({content:'❌ Déjà lancé.',ephemeral:true});
      const r=eng.startTournament(t.id); if(!r.ok) return interaction.reply({content:`❌ ${r.reason}`,ephemeral:true});
      ...
      return interaction.reply({embeds:[new EmbedBuilder().setColor('#E74C3C').setTitle(`⚔️ Tournoi "${t.name}" lancé !`).addFields({name:'🥊 Matchs Round 1',value:list||'—'},{name:'💰 Prize Pool',value:`**${t.prize_pool} ${coin}**`,inline:true}).setFooter({text:'/tournoi jouer pour résoudre'})]});
    }
    if(sub==='jouer'){
      const t=eng.getActiveTournament(guildId);
      if(!t||t.status!=='running'||t.host_id!==userId) return interaction.reply({content:'❌ Pas de tournoi en cours / pas l\'hôte.',ephemeral:true});
      const pending=eng.getPendingMatches(t.id); if(!pending.length) return interaction.reply({content:'❌ Aucun match en attente.',ephemeral:true});
      const results=[];
    ...
    if(sub==='statut'){
      const t=eng.getActiveTournament(guildId); if(!t) return interaction.reply({content:'❌ Aucun tournoi actif.',ephemeral:true});
    ...
    if(sub==='annuler'){
      ...
      return interaction.reply({embeds:[new EmbedBuilder().setColor('#E74C3C').setTitle('❌ Tournoi annulé').setDescription(`Tournoi **${t.name}** annulé.${t.entry_fee>0?` Joueurs remboursés (${t.entry_fee} ${coin}).`:''}`)]}); 
    }
  }
};
