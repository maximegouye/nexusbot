...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('enigme')
    .setDescription('🧩 Résolvez des énigmes et gagnez des coins !')
  ...
  async execute(interaction) {
    ...
    if (sub === 'jouer') {
      if (activeEnigmes.has(key)) return interaction.reply({ content: '❌ Vous avez déjà une énigme en cours ! Répondez avec `/enigme reponse`.', ephemeral: true });
    ...
    if (sub === 'reponse') {
      ...
      if (!game) return interaction.reply({ content: '❌ Aucune énigme en cours. Lancez-en une avec `/enigme jouer` !', ephemeral: true });
      ...
      if (isCorrect) {
        ...
        return interaction.reply({ content: `❌ Mauvaise réponse ! Réessayez ou demandez un \`/enigme indice\`.`, ephemeral: true });
      }
    }

    if (sub === 'indice') {
      const game = activeEnigmes.get(key);
      if (!game) return interaction.reply({ content: '❌ Aucune énigme en cours.', ephemeral: true });
      if (game.hintUsed) return interaction.reply({ content: '❌ Vous avez déjà utilisé votre indice.', ephemeral: true });
