...
module.exports = {
    ...
    .addSubcommand(s => s.setName('canne').setDescription('🎣 Améliorer votre canne à pêche')
      .addStringOption(o => o.setName('action').setDescription('Action').setRequired(true)
        .addChoices(
          ...
          { name: '⬆️ Améliorer (coûte des coins)', value: 'ameliorer' },
        ))),
  ...
  async execute(interaction) {
    ...
    if (sub === 'pecher') {
      ...
      if (cd > 0) return interaction.reply({ content: `⏳ Patience ! Réessayez dans **${cd}s**.`, ephemeral: true });
    ...
    if (sub === 'classement') {
      ...
      if (!top.length) return interaction.reply({ content: '❌ Aucun pêcheur.', ephemeral: true });
      const medals = ['🥇', '🥈', '🥉'];
    ...
    if (sub === 'canne') {
      ...
      if (action === 'ameliorer') {
        if (p.rod_level >= maxLevel) return interaction.reply({ content: '✅ Canne déjà au niveau maximum !', ephemeral: true });
