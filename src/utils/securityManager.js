...
async function securityCheck(interaction, opts = {}) {
  ...
  if (isBlacklisted(userId, guildId)) { await (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '🚫 Vous êtes blacklisté sur ce serveur.', ephemeral: true }).catch(() => {}); return false; }
  if (!skipGlobalRate) { const rate = checkGlobalRate(userId); if (!rate.ok) { await (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: `⏳ Trop de commandes ! Réessaie dans **${rate.waitSec}s**.`, ephemeral: true }).catch(() => {}); return false; } }
  if (requireAdmin && !interaction.member.permissions.has('Administrator')) { await (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '🔒 Réservé aux administrateurs.', ephemeral: true }).catch(() => {}); return false; }
  if (!lockCmd(userId, cmdName)) { await (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '⚠️ Commande déjà en cours.', ephemeral: true }).catch(() => {}); return false; }
  return true;
}
