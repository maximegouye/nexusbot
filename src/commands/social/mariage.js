...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('mariage')
    .setDescription('💍 Système de mariage Discord')
    .addSubcommand(s => s.setName('demander').setDescription('💍 Faire une demande en mariage')
      .addUserOption(o => o.setName('membre').setDescription('Qui tu veux épouser ?').setRequired(true)))
    .addSubcommand(s => s.setName('divorcer').setDescription('💔 Divorcer de ton/ta partenaire'))
    .addSubcommand(s => s.setName('voir').setDescription('👫 Voir qui est marié avec qui')),
  cooldown: 10,
    ...
    if (sub === 'demander') {
      ...
      const msg = await interaction.editReply({
        ...
        embeds: [new EmbedBuilder()
          ...
          .setDescription(`**${interaction.user.username}** te demande en mariage, **${target.username}** ! 🌹\n\nAcceptes-tu de l'épouser ?`)
      ...
      const collector = msg.createMessageComponentCollector({ filter: i => i.user.id === target.id, time: 60000, max: 1 });

      collector.on('collect', async i => {
        if (i.customId === 'marry_refuse') {
          return i.update({
            embeds: [new EmbedBuilder().setColor('#888888').setDescription(`💔 **${target.username}** a refusé la demande... C'est douloureux.`)],
            components: []
          });
        }

        db.db.prepare('INSERT INTO marriages (guild_id, user1_id, user2_id, married_at) VALUES (?, ?, ?, ?)')
          .run(interaction.guildId, interaction.user.id, target.id, Math.floor(Date.now() / 1000));

        await i.update({
          embeds: [new EmbedBuilder()
            .setColor('#FF73FA')
            .setTitle('💍 Félicitations aux mariés ! 🎊')
            .setDescription(`**${interaction.user.username}** 💍 **${target.username}**\n\nVous êtes maintenant officiellement mariés sur ce serveur ! 🌹`)
      ...
      collector.on('end', (_, reason) => {
        if (reason === 'time') msg.edit({ content: '💔 La demande a expiré.', components: [] }).catch(() => {});
      });
    }
