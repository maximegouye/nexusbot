...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(s => s.setName('ajouter').setDescription('➕ Assigner un rôle à un niveau')
      .addIntegerOption(o => o.setName('niveau').setDescription('Niveau requis').setMinValue(1).setRequired(true))
  ...
  async execute(interaction) {
    ...
    if (!interaction.member.permissions.has(PermissionFlagsBits.ManageRoles))
      return await (interaction.deferred||interaction.replied ? interaction.editReply({ content: '❌ Permission insuffisante.', ephemeral: true }) : interaction.reply({ content: '❌ Permission insuffisante.', ephemeral: true }));
    ...
    if (sub === 'ajouter') {
      ...
      if (role.managed) return await (interaction.deferred||interaction.replied ? interaction.editReply({ content: '❌ Ce rôle est géré par une intégration externe.', ephemeral: true }) : interaction.reply({ content: '❌ Ce rôle est géré par une intégration externe.', ephemeral: true }));
      if (role.id === interaction.guild.id) return await (interaction.deferred||interaction.replied ? interaction.editReply({ content: '❌ Tu ne peux pas utiliser le rôle @everyone.', ephemeral: true }) : interaction.reply({ content: '❌ Tu ne peux pas utiliser le rôle @everyone.', ephemeral: true }));
    ...
    if (sub === 'supprimer') {
      ...
      if (!lr) return await (interaction.deferred||interaction.replied ? interaction.editReply({ content: `❌ Aucun level role configuré pour le niveau ${niveau}.`, ephemeral: true }) : interaction.reply({ content: `❌ Aucun level role configuré pour le niveau ${niveau}.`, ephemeral: true }));
    ...
    if (sub === 'liste') {
      ...
      if (!roles.length)
        return await (interaction.deferred||interaction.replied ? interaction.editReply({ content: '❌ Aucun level role configuré sur ce serveur.', ephemeral: true }) : interaction.reply({ content: '❌ Aucun level role configuré sur ce serveur.', ephemeral: true }));
