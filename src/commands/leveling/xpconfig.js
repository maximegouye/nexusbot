...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(s => s.setName('taux').setDescription('Modifier le taux de XP par message')
      .addIntegerOption(o => o.setName('xp_min').setDescription('XP minimum par message (défaut: 15)').setMinValue(1).setMaxValue(500))
      .addIntegerOption(o => o.setName('coins').setDescription('Pièces par message (défaut: 5)').setMinValue(0).setMaxValue(100)))
    ...
    .addSubcommand(s => s.setName('multiplicateur').setDescription('XP x2, x3... pour un rôle (ex: Booster)')
      .addRoleOption(o => o.setName('role').setDescription('Rôle').setRequired(true))
      .addNumberOption(o => o.setName('valeur').setDescription('Multiplicateur (1.0 à 10.0, 0 = supprimer)').setMinValue(0).setMaxValue(10).setRequired(true)))
  ...
  async execute(interaction) {
    ...
    if (sub === 'activer') {
      ...
      return interaction.reply({ embeds: [new EmbedBuilder().setColor(actif ? 'Green' : 'Red').setDescription(`✅ Système XP ${actif ? 'activé' : 'désactivé'}.`)], ephemeral: true });
    }
    ...
    if (sub === 'noxp_canal') {
      ...
      if (ajouter) {
        ...
        return interaction.reply({ embeds: [new EmbedBuilder().setColor('Green').setDescription(`✅ <#${canal.id}> ajouté aux canaux sans XP.`)], ephemeral: true });
      } else {
        db.db.prepare('DELETE FROM no_xp WHERE guild_id=? AND type=? AND target_id=?').run(interaction.guildId, 'channel', canal.id);
        return interaction.reply({ embeds: [new EmbedBuilder().setColor('Green').setDescription(`✅ <#${canal.id}> retiré des canaux sans XP.`)], ephemeral: true });
      }
    }
    ...
    if (sub === 'noxp_role') {
      ...
      if (ajouter) {
        ...
        return interaction.reply({ embeds: [new EmbedBuilder().setColor('Green').setDescription(`✅ <@&${role.id}> ajouté aux rôles sans XP.`)], ephemeral: true });
      } else {
        db.db.prepare('DELETE FROM no_xp WHERE guild_id=? AND type=? AND target_id=?').run(interaction.guildId, 'role', role.id);
        return interaction.reply({ embeds: [new EmbedBuilder().setColor('Green').setDescription(`✅ <@&${role.id}> retiré des rôles sans XP.`)], ephemeral: true });
      }
    }
    ...
    if (sub === 'multiplicateur') {
      ...
      return interaction.reply({ embeds: [new EmbedBuilder().setColor('Green').setDescription(`✅ <@&${role.id}> → **x${valeur}** XP.`)], ephemeral: true });
    }
    ...
    if (sub === 'message_levelup') {
      ...
      return interaction.reply({ embeds: [new EmbedBuilder().setColor('Green').setDescription(`✅ Message de level-up mis à jour:\n${msg}`)], ephemeral: true });
    }
    ...
    if (sub === 'canal_levelup') {
      ...
      return interaction.reply({ embeds: [new EmbedBuilder().setColor('Green').setDescription(canal ? `✅ Level-up → <#${canal.id}>` : '✅ Level-up dans le canal actuel.')], ephemeral: true });
    }
  }
};
