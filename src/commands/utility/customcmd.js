...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'supprimer') {
      ...
      if (!cmd) return interaction.reply({ content: `❌ Aucune commande pour \`${declencheur}\`.`, ephemeral: true });
    ...
    if (sub === 'liste') {
      ...
      if (!cmds.length) return interaction.reply({ content: '❌ Aucune commande personnalisée configurée.', ephemeral: true });
