...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'creer') {
      ...
      if (duree < 10) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Durée trop courte (minimum 10 secondes).', ephemeral: true });
      if (duree > 30 * 86400) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Durée trop longue (maximum 30 jours).', ephemeral: true });
      ...
      await (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '✅ Timer créé !', ephemeral: true });
    ...
    if (sub === 'liste') {
      ...
      if (!timers.length) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Aucun timer actif.', ephemeral: true });
    ...
    if (sub === 'annuler') {
      ...
      if (!timer) return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: `❌ Timer #${id} introuvable.`, ephemeral: true });
      if (timer.created_by !== userId && !interaction.member.permissions.has(0x4000n)) {
        return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Vous ne pouvez annuler que vos propres timers.', ephemeral: true });
      }
      ...
      return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: `✅ Timer **#${id}** annulé.`, ephemeral: true });
    }
  }
};
