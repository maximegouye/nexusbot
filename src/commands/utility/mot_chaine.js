  ...
  async execute(interaction) {
    ...
    if (sub === 'start') {
      if (activeSessions.has(channel.id)) {
        return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '⚠️ Une partie est déjà en cours dans ce salon !', ephemeral: true });
      }
    ...
    if (sub === 'stop') {
      ...
      sess.collector?.stop();
      activeSessions.delete(channel.id);
      return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ embeds: [new EmbedBuilder().setColor('#e74c3c').setDescription('⏹️ Partie arrêtée.')] });
    }
