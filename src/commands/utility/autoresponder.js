...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(s => s.setName('ajouter').setDescription('Ajouter une réponse automatique')
      ...
      .addIntegerOption(o => o.setName('cooldown').setDescription('Cooldown en secondes (0 = aucun)').setMinValue(0).setMaxValue(3600)))
    .addSubcommand(s => s.setName('supprimer').setDescription('Supprimer une réponse automatique')
      .addIntegerOption(o => o.setName('id').setDescription('ID de la réponse').setRequired(true)))
  ...
  async execute(interaction) {
    ...
    if (sub === 'ajouter') {
      ...
      if (count >= max) return interaction.reply({ content: `❌ Maximum ${max} réponses auto${premiumHint}.`, ephemeral: true });
    ...
    if (sub === 'liste') {
      ...
      if (!rows.length) return interaction.reply({ content: 'Aucune réponse automatique configurée.', ephemeral: true });
    ...
    if (sub === 'test') {
      ...
      if (!match) return interaction.reply({ content: '❌ Aucune réponse ne correspond à ce message.', ephemeral: true });
      return interaction.reply({ embeds: [new EmbedBuilder().setColor('Green')
