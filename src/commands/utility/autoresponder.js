...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'ajouter') {
      ...
      if (count >= max) return interaction.editReply({ content: `❌ Maximum ${max} réponses auto${premiumHint}.`, ephemeral: true });
    ...
    if (sub === 'supprimer') {
      ...
      if (!r.changes) return interaction.editReply({ content: '❌ Réponse introuvable.', ephemeral: true });
      return interaction.editReply({ embeds: [new EmbedBuilder().setColor('Red').setDescription(`🗑️ Réponse #${id} supprimée.`)], ephemeral: true });
    }

    if (sub === 'liste') {
      const rows = db.db.prepare('SELECT * FROM autoresponder WHERE guild_id=? ORDER BY id').all(interaction.guildId);
      if (!rows.length) return interaction.editReply({ content: 'Aucune réponse automatique configurée.', ephemeral: true });
    ...
    if (sub === 'test') {
      ...
      if (!match) return interaction.editReply({ content: '❌ Aucune réponse ne correspond à ce message.', ephemeral: true });
      return interaction.editReply({ embeds: [new EmbedBuilder().setColor('Green')
