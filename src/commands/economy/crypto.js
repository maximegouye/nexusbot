...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'marche') {
      return interaction.reply({ embeds: [buildMarketEmbed(cfg, interaction.user.id, interaction.guildId)], components: buildButtons(interaction.user.id) });
    }
    if (sub === 'portefeuille') {
      return interaction.reply({ embeds: [buildWalletEmbed(cfg, interaction.user.id, interaction.guildId, user)], components: buildButtons(interaction.user.id) });
    }
    if (sub === 'acheter') {
      ...
      if (amt == null || amt < 1) return interaction.reply({ content: '❌ Montant invalide.', ephemeral: true });
      try {
    ...
    if (sub === 'vendre') {
      ...
      if (!item) return interaction.reply({ content: `❌ Tu ne possèdes pas de ${sym}.`, ephemeral: true });
      const qty = parseQty(raw, item.amount);
      if (qty == null || qty <= 0) return interaction.reply({ content: '❌ Quantité invalide.', ephemeral: true });
      try {
