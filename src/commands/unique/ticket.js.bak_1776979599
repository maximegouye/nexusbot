  ...
  async execute(interaction) {
    ...
    if (sub === 'claim') {
      ...
      if (ticket.claimed_by)
        return interaction.reply({ content: `⚠️ Ce ticket est déjà pris en charge par <@${ticket.claimed_by}>.`, ephemeral: true });
    ...
    if (sub === 'liste') {
      ...
      if (!tickets.length)
        return interaction.reply({ embeds: [new EmbedBuilder().setColor('#7B2FBE').setDescription('📋 Aucun ticket ouvert en ce moment.')], ephemeral: true });
    ...
    if (sub === 'reopen') {
      ...
      if (!ticket) return interaction.reply({ content: `❌ Aucun ticket fermé trouvé pour <@${targetId}>.`, ephemeral: true });

      const closedHoursAgo = ticket.closed_at ? (Date.now() / 1000 - ticket.closed_at) / 3600 : 999;
      if (closedHoursAgo > 24) return interaction.reply({
    ...
    if (sub === 'blacklist') {
      if (!interaction.member.permissions.has(PermissionFlagsBits.ManageGuild))
        return interaction.reply({ content: '❌ Réservé aux admins (Gérer le serveur).', ephemeral: true });
