...
async function handleSetup(interaction, guildId) {
  ...
  if (!interaction.member.permissions.has(PermissionFlagsBits.Administrator)) {
    return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Seuls les administrateurs peuvent configurer ce système.', ephemeral: true });
  }

  const category = interaction.options.getChannel('categorie-discord');

  if (category.type !== 4) {
    return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Le salon sélectionné doit être une catégorie Discord.', ephemeral: true });
  }
...
async function handleRename(interaction, guildId, userId) {
  ...
  if (!tempChannel) {
    return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Vous n\'avez pas de salon vocal temporaire actif.', ephemeral: true });
  }
  ...
  try {
    ...
    if (!channel) {
      ...
      return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: "❌ Votre salon vocal temporaire n'existe plus.", ephemeral: true });
    }
...
async function handleLimit(interaction, guildId, userId) {
  const limitStr = interaction.options.getString('nombre');
...
async function handleKick(interaction, guildId, userId) {
  ...
  try {
    ...
    if (!member || member.voice.channelId !== channel.id) {
      return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: `❌ ${targetUser.username} n'est pas dans ton salon vocal.`, ephemeral: true });
    }
...
async function handleTransfer(interaction, guildId, userId) {
  ...
  if (newOwner.id === userId) {
    return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ You can\'t transfer ownership to yourself.', ephemeral: true });
  }
...
async function handleDisable(interaction, guildId) {
  // Check admin permissions
  if (!interaction.member.permissions.has(PermissionFlagsBits.Administrator)) {
    return (interaction.deferred||interaction.replied?interaction.editReply:interaction.reply).bind(interaction)({ content: '❌ Seuls les administrateurs peuvent désactiver ce système.', ephemeral: true });
  }
