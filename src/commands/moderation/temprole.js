...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'donner') {
      ...
      if (!ms) return interaction.editReply.bind(interaction)('❌ Durée invalide. Ex: `30m`, `2h`, `7d`');

      const expires = Math.floor((Date.now() + ms) / 1000); // stocké en secondes

      // Donner le rôle
      await member.roles.add(role, raison).catch(e => {
        return interaction.editReply.bind(interaction)(`❌ Impossible d'ajouter le rôle : ${e.message}`);
      });
    ...
    if (sub === 'retirer') {
      ...
      return interaction.editReply.bind(interaction)(`✅ Rôle <@&${role.id}> retiré de <@${member.id}>.`);
    }
    ...
    if (sub === 'liste') {
      ...
      if (!rows.length) return interaction.editReply.bind(interaction)('Aucun rôle temporaire actif.');

      const lines = rows.map(r =>
        `<@${r.user_id}> → <@&${r.role_id}> — expire <t:${r.expires_at}:R>`
      ).join('\n');
