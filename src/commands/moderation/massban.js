...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (!ids.length) return interaction.editReply.bind(interaction)({ content: '❌ Aucun ID valide fourni.' });
    if (ids.length > 50) return interaction.editReply.bind(interaction)({ content: '❌ Maximum 50 IDs à la fois.' });
    ...
    for (const id of ids) {
      ...
      try {
        ...
        if (interaction.isRepliable() && !interaction.replied) {
          interaction.editReply.bind(interaction)({ content: '❌ Une erreur est survenue. Ressaie.', }).catch(() => {});
        }
      }
    }
