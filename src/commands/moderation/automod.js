...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'statut') {
      ...
      return interaction.editReply.bind(interaction)({ embeds: [embed] });
    }
    ...
    if (sub === 'antispam') {
      ...
      return interaction.editReply.bind(interaction)({ embeds: [new EmbedBuilder().setColor(actif ? 'Green' : 'Red')
    ...
    if (sub === 'mauvaismots') {
      ...
      if (action === 'list') {
        return interaction.editReply.bind(interaction)({ embeds: [new EmbedBuilder().setColor('#7B2FBE')
      ...
      if (action === 'clear') {
        ...
        return interaction.editReply.bind(interaction)({ embeds: [new EmbedBuilder().setColor('Green').setDescription('✅ Liste de mots interdits vidée.')] });
      }
      if (!mot) return interaction.editReply.bind(interaction)('❌ Précise un mot.');
      if (action === 'add') {
        ...
        return interaction.editReply.bind(interaction)({ embeds: [new EmbedBuilder().setColor('Green').setDescription(`✅ Mot \`${mot}\` ajouté à la liste.`)] });
      }
      if (action === 'remove') {
        words = words.filter(w => w !== mot);
        db.db.prepare('UPDATE automod_config SET bad_words=? WHERE guild_id=?').run(JSON.stringify(words), interaction.guildId);
        return interaction.editReply.bind(interaction)({ embeds: [new EmbedBuilder().setColor('Green').setDescription(`✅ Mot \`${mot}\` retiré de la liste.`)] });
      }
    }
    ...
    if (sub === 'action') {
      ...
      return interaction.editReply.bind(interaction)({ embeds: [new EmbedBuilder().setColor('Green')
